...
export function ChatStatus() {
  ...
  return (
    <div className="flex items-center gap-3 text-[10px] text-muted-foreground border border-border/40 rounded-full px-2 py-0.5 select-none h-6 box-border mr-2 @max-[220px]:hidden">
      ...
      <div className="w-px h-3 bg-border/50 @max-[360px]:hidden" />
      <div className="flex items-center gap-1.5 @max-[360px]:hidden" title="Total Input Tokens">
        <IconArrowUp className="size-3 opacity-70" />
        <span>{inputTotal.toLocaleString()}</span>
      </div>
      <div className="w-px h-3 bg-border/50 @max-[360px]:hidden" />
      <div className="flex items-center gap-1.5 @max-[360px]:hidden" title="Output Tokens">
